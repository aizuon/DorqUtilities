...
local function _GetSpellCooldown(...)
	...
	if not ok or not info then
		return nil, nil, nil, nil, not ok and "restricted" or nil
	end

	local readOk, startTime, duration, isEnabled, modRate = pcall(function()
	...
	local probeOk = pcall(function()
		local safeStartTime = startTime or 0
		local safeDuration = duration or 0
		local readyAt = safeStartTime + safeDuration
		return readyAt > 0 and safeDuration >= 0 and isEnabled ~= false
	end)
	if not probeOk then
		return nil, nil, nil, nil, "restricted"
	end
...
local TOC_VERSION = string_gsub(C_AddOns.GetAddOnMetadata("DorqUtilities", "Version") or "1.0.0", "wowi:revision", 0)
