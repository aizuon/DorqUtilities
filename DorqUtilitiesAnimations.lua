...
local MSBTMedia = DorqUtilities.Media
...
local EraseTable = DorqUtilities.EraseTable
...
local function LoadFont(fontName)
	local fontPath = DorqUtilities.Media.fonts[fontName]
...
local function DisplayEvent(eventSettings, message, texturePath)
	...
	if soundFile and not currentProfile.soundsDisabled then
		soundFile = sounds[soundFile] or soundFile
		if type(soundFile) == "string" then
			if soundFile ~= "" then
				-- If the sound file doesn't contain any slashes, assume it is in MSBT's sound folder
				if not string_find(soundFile, "\\", nil, true) and not string_find(soundFile, "/", nil, true) then
					soundFile = DEFAULT_SOUND_PATH .. soundFile
				end
				local soundFileLower = string_lower(soundFile)
				-- If the sound file doesn't begin with "Interface", don't bother trying
				if (string_find(soundFileLower, "interface", nil, true) or 0) ~= 1 then
					return
				end
